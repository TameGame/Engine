function makeSpaceLocation(bounds) {
    return { 
        tile: { 
            quad: TameGame.bbToQuad(bounds),
            zIndex: 0
        },
        presence: {
            shape: null
...
QUnit.test("SimpleSpaceCanAddObject", function (assert) {
    ...
    var someRef = tree.addObject({}, makeSpaceLocation({ x: 0.5, y: 0.5, width: 0.25, height: 0.25 }));
...
QUnit.test("SimpleSpaceCanFindHugeObject", function (assert) {
    var tree    = new TameGame.SimpleSpace();
    var someRef = tree.addObject({}, makeSpaceLocation({ x: -100, y: -100, width: 200, height: 200 }));            // Forces the space to expand outward
    var count   = 0;
...
function populateGrid(space, width, height) {
    ...
    for (y=0; y<height; ++y) {
        xpos = 0;
        for (x=0; x<width; ++x) {
            space.addObject({ x: xpos, y: ypos }, makeSpaceLocation({ x: xpos, y: ypos, width: distX, height: distY }));
...
QUnit.test("SimpleSpaceQuery100ObjectsExpandTree", function (assert) {
    ...
    tree.addObject({}, makeSpaceLocation({ x: -2, y: -2, width: .5, height: .5 }));                // Shouldn't appear in the test; also expands a populated tree    
...
QUnit.test("SimpleSpaceQuery100ObjectsPlusSomeMore", function (assert) {
    ...
    tree.addObject({}, makeSpaceLocation({ x: .1, y: .1, width: .2, height: .2 }));                // In the 3x3 square
    tree.addObject({}, makeSpaceLocation({ x: .5, y: .5, width: .5, height: .5 }));                // In the 10x10 square
